# 使用 uv run --group build pyinstaller --clean --noconfirm packaging\windows\coinpilot-ai.spec
...
metadata = runpy.run_path(str(root / "coinpilot_ai" / "core" / "version.py"))
version, windows_version = metadata["VERSION"], metadata["WINDOWS_VERSION"]
file_version = ".".join(map(str, windows_version))
manifest = (root / "packaging/windows/coinpilot-ai.manifest").read_text(encoding="utf-8").replace("@APP_VERSION@", file_version)
