...
def test_local_ui_normal_confirmation_flow_and_compact_account_panel(paper, app, monkeypatch):
    from coinpilot_ai.trading import panel as ui_trade
...
def test_local_stop_stop_limit_and_trailing_orders(paper):
    stop = submit(paper, order_type='stop', trigger_price='61000', size='1')
    assert stop['status'] == 'live'
    quote(paper, '60500')
    assert not paper.positions
    quote(paper, '61000')
    assert len(paper.positions) == 1

    stop_limit = submit(paper, order_type='stop_limit', trigger_price='62000', price='61800', size='1')
    assert stop_limit['status'] == 'live'
    quote(paper, '62000')
    assert len(paper.positions) == 1
    quote(paper, '61800')
    assert paper.positions[0]['pos'] == '2'

    trailing = submit(paper, order_type='trailing_stop', trailing_offset='100', size='1')
    quote(paper, '61600')
    assert paper.positions[0]['pos'] == '2'
    quote(paper, '61700')
    assert paper.positions[0]['pos'] == '3'
