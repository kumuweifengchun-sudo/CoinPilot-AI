...
from coinpilot_ai.review.ai_panel import AiPanel
...
def test_chart_layout_switches_fill_available_space(workspace, app, sizes):
    window, _ = workspace
    multi = window.multi_chart
    for size in sizes:
        multi.layout_choice.setCurrentIndex(multi.layout_choice.findData(size))
        app.processEvents()
        rows, columns = multi.SHAPES[size]
        visible = multi.panels[:size]
        assert all(panel.isVisible() for panel in visible)
        assert all(not panel.isVisible() for panel in multi.panels[size:])
        area = multi.grid_host.contentsRect()
        assert visible[0].geometry().topLeft() == area.topLeft()
        assert visible[-1].geometry().bottomRight() == area.bottomRight()
        assert len({panel.y() for panel in visible}) == rows
        assert len({panel.x() for panel in visible}) == columns
        if size == 1:
            assert window.chart.geometry() == area
            assert not window.chart.compact_bar.isVisible()
            assert all(button.isVisible() for button in window.chart.period_buttons.values())


def test_six_chart_contexts_and_named_workspace_restore(workspace, app):
    window, service = workspace
    grid = window.multi_chart
    grid.layout_choice.setCurrentIndex(grid.layout_choice.findData(6))
    app.processEvents()
    assert len(window.findChildren(ChartPanel)) == 6
    second = grid.panels[1]
    second.set_local_pair('ETH-USDT-SWAP', '1H')
    indicator = {"id": "pane-ema", "kind": "EMA", "params": {"period": 12},
                 "color": "#123456", "width": 1.5, "visible": True}
    second.indicator_book.save_indicators([indicator])
    assert service.chart_book.indicators != second.indicator_book.indicators
    second.indicator_book.save_indicators([dict(indicator, bar="4H")])
    assert ('ETH-USDT-SWAP', '4H') in service.indicator_pairs
    second.indicator_book.save_indicators([indicator])
    view_key = 'pane1/demo/ETH-USDT-SWAP/1H'
    service.store.put('chart_pane_view', view_key, {"count": 42, "left_time": 1000,
        "follow": False, "auto": True, "low": 0, "high": 1, "volume_ratio": .22})
    assert second.pair == ('ETH-USDT-SWAP', '1H')
    assert window.chart.pair == (service.selected, service.bar)
    assert second.pair in service.chart_pairs
    second.derivative_button.click()
    app.processEvents()
    assert second.derivative_scroll.isVisible()
    second.derivative_button.click()
    profiles = window.workspace.profiles
    window.scanner_panel.filters["rsi_max"].setText("25")
    profiles.save('六图研究')
    grid.layout_choice.setCurrentIndex(grid.layout_choice.findData(1))
    window.scanner_panel.filters["rsi_max"].setText("70")
