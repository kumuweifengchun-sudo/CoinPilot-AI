...
from .ai import AiClient, PromptLibrary, render_prompt
from .alerts import AlertEngine, RuleState, validate_rule
from .domain import instrument_id, number, aligned
...
from .transport import ApiError, JsonTransport
from .chart_state import ChartBook
from .chart_feed import ChartFeed
        ...
        for key, rule in self.rules():
            ...
            try:
                event = self.engine.evaluate(key, rule, self.quotes.get(inst), self.positions, fresh_candles,
                                             now, now - self.account.get("time", 0) <= 20)
