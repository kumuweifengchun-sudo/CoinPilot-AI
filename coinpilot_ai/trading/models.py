...
class OrderDraft:
    ...
    def payload(self, spec, account, positions, quote, now, client_id, *, paper=False):
        ...
        allowed_types = ("market", "limit", "stop", "stop_limit", "trailing_stop") if paper else ("market", "limit")
        if self.order_type not in allowed_types or self.margin not in ("cross", "isolated"):
        ...
        if self.order_type in ("stop", "stop_limit"):
            payload["triggerPx"] = str(aligned(self.trigger_price, spec["tickSz"], "触发价"))
            if self.order_type == "stop":
                price = number(payload["triggerPx"])
        if self.order_type == "trailing_stop":
            payload["trailOffset"] = str(number(self.trailing_offset, "追踪距离", positive=True))
        protection = {}
        if self.action == "open":
