...
class ChartFeed(QObject):
    ...
    def indicator(self, series, spec):
        """同一合约周期与数据版本只计算一次相同参数的指标。"""
        from coinpilot_ai.market.indicators import calculate, validate_spec
        normalized = validate_spec(spec)
        pair = series.pair
        cached_revision, values = self.indicator_cache.get(pair, (-1, {}))
        if cached_revision != series.revision:
            values = {}
            self.indicator_cache[pair] = (series.revision, values)
        key = (normalized["kind"], tuple(sorted(normalized["params"].items())))
        if key not in values:
            values[key] = calculate(series.data.rows, normalized, bar=pair[1])
        return values[key]
    ...
    def activate(self):
        ...
        wanted = (set(getattr(self.service, "chart_pairs", set())) |
                  set(getattr(self.service, "indicator_pairs", set()))) - {(self.service.selected, self.service.bar)}
